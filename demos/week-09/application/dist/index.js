import { State, GameControls } from "./types.js";
...
class Game {
        ...
        document.addEventListener("keydown", (event) => {
            ...
            switch (key) {
                ...
                case GameControls.PADDLE_1_UP:
                    paddle_1.style.top =
                        Math.max(paddle_1_coord.top -
                            this.SPEED * window.innerHeight, board_coord.top) + "px";
                    paddle_1_coord = paddle_1.getBoundingClientRect();
                    break;
                case GameControls.PADDLE_1_DOWN:
                    paddle_1.style.top =
                        Math.min(paddle_1_coord.top +
                            this.SPEED * window.innerHeight, board_coord.bottom - paddle_1_coord.height) + "px";
                    paddle_1_coord = paddle_1.getBoundingClientRect();
                    break;
                case GameControls.PADDLE_2_UP:
                    paddle_2.style.top =
                        Math.max(paddle_2_coord.top -
                            this.SPEED * window.innerHeight, board_coord.top) + "px";
                    paddle_2_coord = paddle_2.getBoundingClientRect();
                    break;
                case GameControls.PADDLE_2_DOWN:
                    paddle_2.style.top =
                        Math.min(paddle_2_coord.top +
                            this.SPEED * window.innerHeight, board_coord.bottom - paddle_1_coord.height) + "px";
                    paddle_2_coord = paddle_2.getBoundingClientRect();
                    break;
            }
    ...
    moveBall(velocity) {
        ...
        if (ball_coord.left <= paddle_1_coord.left &&
